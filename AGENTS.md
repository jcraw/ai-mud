...
|------|---------|--------------|
| Default / fast | `./tools/verify_mud.sh` or `--fast` | `:core:compileKotlin` smoke; with module args → compile (+ test if `src/test` exists); then **detekt** + **Konsist arch** + **test-lock** + **no_live_llm_unit** + **token hard-on-touched** (never PIT) |
| Core | `./tools/verify_mud.sh --core` | `:core:test :perception:test :memory:test :reasoning:test` (default `excludeTags("quarantine")`) + detekt + Konsist arch + test-lock + no_live_llm_unit + token hard-on-touched (never PIT) |
| Full | `./tools/verify_mud.sh --full` | Stable green set including green `:reasoning` (exclude quarantine); no testbot thrash; + detekt + Konsist arch + test-lock + no_live_llm_unit + token hard-on-touched; PIT skipped (core &gt;45s — use `--pitest`) |
| PIT | `./tools/verify_mud.sh --pitest` | Pure-module PIT (`:core` / `:perception` / `:memory`) + detekt + Konsist arch + test-lock + no_live_llm_unit; soft 60% (see **`docs/PIT.md`**); token skipped |
| Quarantine | `./tools/verify_mud.sh --quarantine` | `:reasoning:test -Pmud.quarantineOnly=true` — debt only; hard-fail OK (no detekt / no Konsist / no test-lock / no no_live_llm_unit / no PIT / no token) |
...
**Detekt** is live on default/fast/core/full/pitest (`./gradlew detekt`). Legacy soft via baseline; mass baseline regen = **Jason/explicit only**. See **`docs/DETEKT.md`**. **Konsist** architecture tests are live on default/fast/core/full/pitest (`:core:test --tests 'com.jcraw.mud.architecture.*'`); exceptions → **`docs/KONSIST.md`**. **Test-lock** is live on default/fast/core/full/pitest (`./tools/test_lock.sh --check`); unauthorized `src/test` edits fail closed — see **`docs/TEST_LOCK.md`**. **No live LLM in unit tests** (`no_live_llm_unit`) is hard on default/fast/core/full/pitest (`./tools/quality/check_no_live_llm_unit.sh`); forbids real `OpenAIClient(` / API-key load under `*/src/test/**`; hard-excludes `testbot/**`; quarantine skips — see **`docs/NO_LIVE_LLM_UNIT.md`**. **PIT** mutation on pure modules (`:core` / `:perception` / `:memory`) via `./tools/verify_mud.sh --pitest` only (default/fast/core never; full skips — core measured &gt;45s). Soft 60% now; schedule 60→70→80 → **`docs/PIT.md`**. Hard opt-in `MUD_PITEST_HARD=1`. **Token/structure** is **hard-on-touched** on default/fast/core/full (scoped git-diff `*_E` → fail; findings → dod-summary; `gates.token_budget`); soft opt-out `MUD_TOKEN_SOFT=1` / `--token-soft`; `MUD_TOKEN_SCOPE=full` soft inventory only; overrides require burn-down ticket and **may only lower** over time (new/Added files cannot use overrides); quarantine/pitest skip — see **`docs/TOKEN_BUDGET_KT.md`**.
